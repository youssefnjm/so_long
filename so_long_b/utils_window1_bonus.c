...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   utils_window1_bonus.c                              :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: ynoujoum <marvin@42.fr>                    #+#  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025-01-15 12:51:39 by ynoujoum          #+#    #+#             */
/*   Updated: 2025-01-15 12:51:39 by ynoujoum         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	render_floor(t_info *info)
{
	...
	while (y < info->height - 1)
	{
		...
		while (x < info->width - 1)
		{
			...
			if (info->map_cpy[y][x] == 'M' || info->map[y][x] == '0')
	...
	while (y < info->height - 1)
	{
		...
		while (x < info->width - 1)
		{
			if (info->map[y][x] == 'P')
				...
				put_img(info, info->collectible[info->frame], x, y);
			else if (info->map[y][x] == 'S')
				put_img(info, info->snowman, x, y);
			x++;
		}
		y++;
	}
...
int	render_game(t_info *info)
{
	...
	if (init_images(info) == 1)
		return (1);
	mlx_loop_hook(info->mlx, handle_animation, info);
